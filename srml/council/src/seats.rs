...
use rstd::prelude::*;
use primitives::traits::{Zero, One, As, StaticLookup, Staking};
...
type BalanceOf<T> = <<T as democracy::Trait>::Staking as Staking<<T as system::Trait>::AccountId>>::Balance;
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		fn set_approvals(origin, votes: Vec<bool>, #[compact] index: VoteIndex) {
			...
			if !<LastActiveOf<T>>::exists(&who) {
				...
				T::Staking::reserve(&who, Self::voting_bond())?;
		...
		fn reap_inactive_voter(
			...
			if valid {
				...
				T::Staking::repatriate_reserved(&who, &reporter, Self::voting_bond())?;
				Self::deposit_event(RawEvent::VoterReaped(who, reporter));
			} else {
				T::Staking::slash_reserved(&reporter, Self::voting_bond());
		...
		fn retract_voter(origin, #[compact] index: u32) {
			...
			T::Staking::unreserve(&who, Self::voting_bond());
		}
		...
		fn submit_candidacy(origin, #[compact] slot: u32) {
			...
			T::Staking::reserve(&who, Self::candidacy_bond())
		...
		fn present_winner(
			...
			ensure!(T::Staking::can_slash(&who, bad_presentation_punishment), "presenter must have sufficient slashable funds");
			...
			if total == actual_total && !dupe {
				...
				let _ = T::Staking::slash(&who, bad_presentation_punishment);
...
impl<T: Trait> Module<T> {
	...
	fn start_tally() {
		...
		if retaining_seats < desired_seats {
			...
			let voters = Self::voters();
			let votes = voters.iter().map(T::Staking::total_balance).collect::<Vec<_>>();
	...
	fn finalise_tally() -> Result {
		...
		let incoming: Vec<T::AccountId> = leaderboard.iter()
			...
			.map(|(_, a)| a)
			.cloned()
			.inspect(|a| {T::Staking::unreserve(a, candidacy_bond);})
			.collect();
		let active_council = Self::active_council();
